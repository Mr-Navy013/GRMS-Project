...
import "../../styles/NonTeachingStaffDashboard.css";
...
const NonTeachingStaffDashboard = ({ user }) => {
  const universityName = user?.university || user?.college || "OUTR University";
  const userEmail = user?.email || "nonteachingstaff@gmail.com";
  ...
  const [userName, setUserName] = useState(user?.name || user?.email || "Non-Teaching Staff Member");
  ...
  const reloadGrievances = () => {
    ...
    setWorkOrders(allUni.filter((g) => g.email === userEmail || g.role === "Non-Teaching Staff"));
  };
  ...
  const handleFormSubmit = (e) => {
    ...
    if (editingId) {
      ...
      setModalAlert("Order updated successfully!");
    } else {
      const createdData = {
        id: `NTS-${Math.floor(300 + Math.random() * 700)}`,
      ...
      reloadGrievances();
      setModalAlert("Staff grievance order submitted to Admin!");
    }
  ...
  const pendingCount = workOrders.filter((g) => g.status === "Pending").length;
  const inProgressCount = workOrders.filter((g) => g.status === "In Progress").length;
  const resolvedCount = workOrders.filter((g) => g.status === "Resolved").length;

  const filteredOrders = workOrders.filter((g) =>
    statusFilter === "All" ? true : g.status === statusFilter
  );

  const firstName = userName ? userName.trim().split(" ")[0] : "Staff";
  ...
  const jobRole = user?.jobRole || "Clerical Staff";
  const joiningYear = user?.joiningYear || "2015";
    ...
    <div className="unified-dashboard">
      ...
      <div className="dashboard-main-container">
        ...
        {activeView === "dashboard" && (
          <div className="card-panel">
            <div className="panel-header">
              <h3>{statusFilter === "All" ? "Staff Grievance Log" : `Filtered: ${statusFilter}`}</h3>
              {statusFilter !== "All" && (
                  ...
                  {filteredOrders.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="6" style={{ textAlign: "center", padding: "30px", color: "#64748b" }}>
                        No orders found under '{statusFilter}' status filter.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        {activeView === "profile" && (
          <div className="card-panel">
            <div className="panel-header">
              <h3>Non-Teaching Staff Profile Information</h3>
            </div>
            <div className="stats-grid">
              ...
              <div className="stat-card">
                <h4>Designation & Job Role</h4>
                <p style={{ margin: 0, fontWeight: "700", fontSize: "1.1rem" }}>{jobRole}</p>
              </div>
              <div className="stat-card">
                <h4>Joining Year</h4>
            ...
            <div className="modal-dialog-body">
              <form onSubmit={handleSaveProfileModal}>
                ...
                <div className="form-group-custom">
                  ...
                  <DragDropUpload
                    onFileSelected={(file) => console.log("Staff photo uploaded:", file)}
                    accept="image/*"
                  />
                </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={handleCancelClick}>
          <div className="modal-dialog" onClick={(e) => e.stopPropagation()}>
            <div className="modal-dialog-header">
              <h3>{editingId ? `Edit Order (${editingId})` : "Submit Staff Grievance"}</h3>
